...
public class RunTimerActivity extends BaseActivity {
	
	private static final String TAG = "IT: RunTimerActivity";
	private DbHelper db;
...
//		((TextView)findViewById(R.id.exerciseTitleTv2)).setText(currentExercise.toString());
	...
	private void startTimer() {
		...
		if (exTimeLeft == 0) {
			...
			if (unitProgress == 0) {
				timerCycle.setText(""+cycle+" / "+currentUnit.getTime(Time.REPETITIONS));
			...
			cdTimer = new CountDownTimer(exTimeLeft * 1000, 1000) {
				public void onTick(long millisUntilFinished) {
					totalTimeLeft -= 1;
					exTimeLeft -= 1;
					timerTotalTimeLeft.setText(BaseActivity.timeString(totalTimeLeft, true));
		            timerCurrentTimeLeft.setText(BaseActivity.timeString((int)millisUntilFinished / 1000, true));
		        	if (exTimeLeft == 5) {
		        		play(COUNT);
		        	}
		        }
		        public void onFinish() {
		        	totalTimeLeft--;
		        	timerTotalTimeLeft.setText(BaseActivity.timeString(totalTimeLeft, true));
		        	timerCurrentTimeLeft.setText(BaseActivity.timeString(0, true));
		            if (unitProgress < currentUnit.getCount() - 1) {
		            	unitProgress++;
		        		exTimeLeft = currentUnit.getTimesArray()[unitProgress];
		            	startTimer();
